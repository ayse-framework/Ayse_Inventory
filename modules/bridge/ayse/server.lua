...
AddEventHandler("Ayse:characterUnloaded", playerDropped)

RegisterNetEvent("Ayse:jobChanged", function(source, job, lastJob)
...
SetTimeout(500, function()
    ...
    server.GetPlayerFromId = AyseCore.Functions.GetPlayer
    for _, character in pairs(AyseCore.Functions.GetPlayers()) do
...
RegisterNetEvent("Ayse:moneyChange", function(player, account, amount, changeType)
...
function server.syncInventory(inv)
    ...
    if money then
        local character = AyseCore.Functions.GetPlayer(inv.id)
        AyseCore.Functions.SetPlayerData(character.id, "cash", money.money)
    end
end
...
function server.buyLicense(inv, license)
	...
	AyseCore.Functions.CreatePlayerLicense(inv.owner, "weapon")
